...
async function loadProduct() {
  ...
  try {
    ...
    formData.code = product.value.code
    ...
    formData.costPrice = product.value.costPrice
    formData.minimumStockLevel = product.value.minimumStockLevel
    formData.currentStockLevel = product.value.currentStockLevel || null
    formData.weight = product.value.weight || null
    formData.dimensions = product.value.dimensions || ''
    formData.isActive = product.value.isActive

    uiStore.setBreadcrumbs([
      { label: t('nav.inventory'), to: '/inventory' },
      { label: t('products.title'), to: '/inventory/products' },
      { label: product.value.name, to: `/inventory/products/${product.value.id}` },
      { label: t('common.edit') },
    ])
  }
  catch (error) {
...
async function handleSubmit() {
  ...
  try {
    ...
    toast.showSuccess(t('messages.success_update'), t('products.updated_successfully'))
...
function cancel() {
  router.push(`/inventory/products/${route.params.id}`)
}
