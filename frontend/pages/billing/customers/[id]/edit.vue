...
async function loadCustomer() {
  ...
  try {
    ...
    uiStore.setBreadcrumbs([
      ...
      { label: customer.value.name, to: `/billing/customers/${customer.value.id}` },
      { label: t('common.edit') },
    ])
  }
  catch (error) {
...
async function handleSubmit() {
  ...
  try {
    ...
    toast.showSuccess(t('messages.success_update'), t('customers.updated_successfully'))
    router.push(`/billing/customers/${customer.value.id}`)
  }
  catch (error) {
    const errMessage = error instanceof Error ? error.message : 'Unknown error'
...
function cancel() {
  router.push(`/billing/customers/${route.params.id}`)
}
